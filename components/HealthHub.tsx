  ...
  useEffect(() => {
    if (activeBooking) {
      const tips = activeBooking.altitude === 'High' 
        ? ['Acclimatize slowly.', 'Drink 5L of water daily.', 'Check oxygen levels often.']
        : ['Use sunscreen SPF 50+.', 'Drink only bottled water.', 'Keep allergy meds ready.'];
      setSyncTips(tips);
    }
  }, [activeBooking]);
  ...
  return (
    <div className="space-y-10 animate-fadeIn pb-20">
      <div className="flex justify-between items-center mb-6">
         <h2 className="text-3xl font-bold text-[#2a1b18] font-serif tracking-tighter uppercase italic">Nexo<span className="text-[#c4b5fd]">.Health</span></h2>
      </div>

      {activeBooking && (
        <div className="bg-[#2a1b18] rounded-[3.5rem] p-12 text-[#f8f5f0] shadow-2xl relative overflow-hidden group">
           <div className="relative z-10">
              <span className="bg-[#f8f5f0]/10 border border-white/10 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest text-[#c4b5fd] font-serif">Travel Health Tips</span>
              <h3 className="text-3xl font-bold mt-6 uppercase tracking-tighter font-serif italic">Trip to: {activeBooking.destination}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-10">
           <div className="bg-white border border-black/5 p-12 rounded-[3.5rem] shadow-xl">
             <h3 className="text-xl font-bold text-[#2a1b18] mb-10 flex items-center gap-5 uppercase tracking-tighter font-serif italic">
               <i className="fa-solid fa-microscope text-[#c4b5fd]"></i> Symptom Checker
             </h3>
             <textarea 
               value={symptoms}
             ...
             <button onClick={handleDiagnose} disabled={loading} className="mt-10 w-full h-[72px] bg-[#2a1b18] hover:bg-[#4a4a4a] text-[#f8f5f0] rounded-2xl font-black uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-4">
               {loading ? <i className="fa-solid fa-spinner fa-spin"></i> : <span>CHECK SYMPTOMS</span>}
             </button>
           </div>
             ...
             <div className="bg-white border-2 border-dashed border-[#c4b5fd] p-12 rounded-[3.5rem] shadow-2xl animate-slideUp">
               <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
                 <div>
                   ...
                   <p className="text-[#c4b5fd] font-black uppercase tracking-[0.25em] text-[10px] mt-2 italic">Urgency: {result.urgency}</p>
                 </div>
                 <div className="bg-[#f5f2eb] px-8 py-4 rounded-2xl border border-black/5 shadow-sm">
                   <p className="text-[10px] font-black text-[#4a4a4a] uppercase tracking-widest mb-1">Recommended Specialist</p>
               ...
               <div className="space-y-10">
                 <div>
                    <label className="text-[10px] font-black text-[#4a4a4a] uppercase tracking-widest mb-6 block font-serif">Recommended Hospitals</label>
                      ...
                      {result.hospitals?.map((h, i) => (
                        <div key={i} className="bg-[#f5f2eb] p-6 rounded-2xl border border-black/5 flex items-center gap-5 hover:border-[#c4b5fd] transition-all group">
                           <div className="h-10 w-10 rounded-xl bg-white text-[#2a1b18] flex items-center justify-center shadow-sm group-hover:bg-[#2a1b18] group-hover:text-white transition-colors">
                             <i className="fa-solid fa-building-h text-base"></i>
                           </div>
        ...
        <div className="bg-[#f5f2eb] border border-black/5 rounded-[3.5rem] p-12 h-fit space-y-10 shadow-sm">
           <h4 className="font-bold text-xl text-[#2a1b18] uppercase tracking-tighter font-serif italic border-b border-black/10 pb-6">Consult a Doctor</h4>
           <div className="space-y-6">
             {[
               { n: 'Dr. Binod Poudel', s: 'Cardiology', h: 'TUTH Central' },
               { n: 'Dr. Sita Gurung', s: 'General Medicine', h: 'Rescue Center' },
               { n: 'Dr. Amit Shah', s: 'Internal Medicine', h: 'Medicity' },
             ].map((doc, i) => (
           ...
           <button className="w-full bg-[#2a1b18] text-white py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#4a4a4a] transition-all shadow-xl font-serif">ONLINE CONSULTATION</button>
        </div>
      </div>
    </div>
  );
