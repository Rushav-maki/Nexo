...
const AgriClimate: React.FC = () => {
  ...
  const [tab, setTab] = useState<'Analysis' | 'Seeds'>('Analysis');
  const [loading, setLoading] = useState(false);

  const seeds: Seed[] = [
    { name: 'Khumal-Rice 4', category: 'Rice', region: 'Hills', yield: '5.5 t/ha' },
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-20">
      <div className="flex bg-[#f5f2eb] p-1.5 rounded-[1.5rem] border border-black/5 w-fit shadow-inner">
        <button onClick={() => setTab('Analysis')} className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${tab === 'Analysis' ? 'bg-[#2a1b18] text-[#f8f5f0] shadow-md' : 'text-[#4a4a4a] hover:text-[#2a1b18]'}`}>Farming Tips</button>
        <button onClick={() => setTab('Seeds')} className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${tab === 'Seeds' ? 'bg-[#2a1b18] text-[#f8f5f0] shadow-md' : 'text-[#4a4a4a] hover:text-[#2a1b18]'}`}>Seed Info</button>
      </div>

      <div className="flex justify-between items-center mb-6">
         <h2 className="text-3xl font-bold text-[#2a1b18] font-serif tracking-tighter uppercase italic">Nexo<span className="text-[#c4b5fd]">.Agro</span></h2>
      </div>
      ...
      {tab === 'Analysis' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           <div className="lg:col-span-2 space-y-8">
             <div className="bg-white border border-black/5 p-12 rounded-[3.5rem] shadow-xl">
               <h3 className="text-xl font-bold text-[#2a1b18] mb-10 flex items-center gap-4 uppercase tracking-tighter font-serif italic">
                 <i className="fa-solid fa-leaf text-[#c4b5fd]"></i> Crop Advice
               </h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 ...
                 <div className="space-y-2">
                   <label className="text-[10px] font-black text-[#4a4a4a] uppercase tracking-widest ml-1">What are you planting?</label>
               ...
               <button onClick={handleAnalyze} disabled={loading} className="mt-10 w-full h-[72px] bg-[#2a1b18] hover:bg-[#4a4a4a] text-[#f8f5f0] rounded-2xl font-black uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-4">
                 {loading ? <i className="fa-solid fa-spinner fa-spin"></i> : <span>GET ADVICE</span>}
               </button>
             </div>
               ...
               <div className="bg-white border-2 border-dashed border-[#c4b5fd] p-12 rounded-[3.5rem] shadow-2xl animate-slideUp">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                     <div className="space-y-8">
                        <div>
                          <p className="text-[10px] font-black text-[#c4b5fd] uppercase tracking-widest mb-1 font-serif">Suitability</p>
                          <p className="text-2xl font-bold text-[#2a1b18] uppercase tracking-tighter italic font-serif">{report.suitability}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black text-[#c4b5fd] uppercase tracking-widest mb-1 font-serif">Best Variety</p>
           ...
           <div className="bg-[#2a1b18] p-12 rounded-[3.5rem] shadow-2xl text-[#f8f5f0] h-fit relative overflow-hidden group">
              <h4 className="font-bold text-xl text-white mb-10 uppercase tracking-tighter font-serif italic border-b border-white/5 pb-6">Market Prices</h4>
              <div className="space-y-5 relative z-10">
                {[
                  { n: 'Potato (Red)', p: 'Rs. 45', t: 'down' },
                  { n: 'Tomato', p: 'Rs. 85', t: 'up' },
          ...
          {seeds.map((s, i) => (
            <div key={i} className="bg-white border border-black/5 p-10 rounded-[3.5rem] hover:border-[#c4b5fd] transition-all group shadow-sm hover:shadow-xl">
              ...
              <div className="mt-12 pt-8 border-t border-black/5 flex justify-between items-center">
                 <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">{s.yield} Yield</span>
                 <button className="text-[#2a1b18] text-[10px] font-black uppercase tracking-widest hover:text-[#c4b5fd] transition-colors">Details</button>
              </div>
            </div>
          ))}
        </div>
